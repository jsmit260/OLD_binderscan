...
class IPer():
    def __init__(self, ip_range):
        self.ip_range = ipaddress.IPv4Network('192.168.0.1/24')
    
    def get_ips(netrange):
        hosts = list(ipaddress.ip_network(netrange).hosts())
        ten_percent = len(hosts)*.1 # adjusted from 10% for C nets
...
operating_systems = ["minimal/trusty64", # SIMPLE LINUX SERVER
                    "minimal/centos7", # SIMPLE LINUX SERVER
                    "minimal/xenial64", # SIMPLE LINUX SERVER
                    "minimal/wheezy64", # SIMPLE LINUX SERVER
                    "stevs986/ubuntu16.04-webserver", # WEB SERVER
                    "bseller/oracle-standard", # DATABASE SERVER
                    "jquintero/vm_ftp_server", # FTP SERVER
                    "scotch/box", # LAMP STACK
                    "richdynamix/magestead-centos65-nginx-php56"] # EMAIL SERVER
...
with open(path+'VagrantFile_Template') as f:
    ...
    with open(path+'hostnames.txt','w+') as f:
        for hostname in host_names:
            f.write(hostname+'\n')
    #port_map = range(3000, 13000)
    #port_map_str = ','.join(str(e) for e in port_map)
    ...
    #new_port_mapping = new_N_hosts.replace('PORTY_PORTS',port_map_str) 
